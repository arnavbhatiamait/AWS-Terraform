...
resource "aws_route" "secondary_to_primary_route" {
  ...
  vpc_peering_connection_id = aws_vpc_peering_connection_accepter.secondary_to_primary.id
...
resource "aws_security_group" "primary_sg" {
  ...
  ingress {
    # ! ICMP -> Protocol number for ICMP is 1, but in AWS security group we use -1 to allow all ICMP types and codes
...
resource "aws_instance" "primary_instance" {
  ...
  tags = {
    Name        = "primary-instance-${var.primary_reg}"
    Environment = "Demo"
    tag         = var.primary_reg
  }
  depends_on = [aws_vpc_peering_connection_accepter.secondary_to_primary]
...
resource "aws_instance" "secondary_instance" {
  ...
  tags = {
    Name        = "secondary-instance-${var.secondary_reg}"
    Environment = "Demo"
    tag         = var.secondary_reg
  }
  depends_on = [aws_vpc_peering_connection.primary_to_secondary]
}